...
namespace FingerprintRecognitionV2.Util.Preprocessing
{
    static public class MinutiaeExtractor
    {
        ...
        static public List<Minutiae> Extract(bool[,] gabor, double[,] orient, int wl, int bs)
        {
            ...
            bool[,] msk = new bool[gabor.GetLength(0), gabor.GetLength(1)];

            Iterator2D.Forward(gabor, (y, x) =>
            {
         ...
         * */
        static private int CountNeighbors(bool[,] img, int y, int x)
        {
            var p2 = img[y - 1, x];
            var p3 = img[y - 1, x + 1];
            var p4 = img[y, x + 1];
            ...
            return CountTrue(p2, p3, p4, p5, p6, p7, p8, p9);
        }
